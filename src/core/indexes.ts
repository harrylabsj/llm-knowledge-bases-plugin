...
import type { DerivedNoteKind, KnowledgeBasePluginConfig, RawKind, RunLogEntry } from "../types.js";
...
async function collectSourceEntries(config: KnowledgeBasePluginConfig): Promise<SourceIndexEntry[]> {
  ...
  return sourceFiles.map(({ path: notePath, content }) => {
    ...
    const inferredRawKind = getSupportedRawFileInfo(frontmatter.raw_path)?.rawKind ?? "text";
    return {
      id: frontmatter.id,
      notePath,
      title: frontmatter.title,
      rawPath: frontmatter.raw_path,
      rawKind: frontmatter.raw_kind ?? inferredRawKind,
...
function buildSourcesIndexMarkdown(indexDir: string, entries: SourceIndexEntry[]): string {
  ...
  for (const entry of entries.sort((a, b) => a.id.localeCompare(b.id))) {
    ...
    lines.push(
      `- [${entry.title}](${relativeLink}) (\`${entry.id}\`) [source | ${entry.rawKind}] - ${entry.summary || "_No summary yet._"} - raw: \`${entry.rawPath}\` - updated: ${entry.updatedAt}`,
    );
  }
...
function renderCatalogSection(
  wikiRoot: string,
  entries: Array<{ notePath: string; title: string; id: string; summary: string; badge?: string }>,
): string {
  if (entries.length === 0) {
    return "_None yet._";
  }
  ...
  return entries
    ...
    .map((entry) => {
      ...
      return `- [${entry.title}](${relativeLink}) (\`${entry.id}\`)${badge}: ${entry.summary || "_No summary yet._"}`;
    })
    .join("\n");
}
